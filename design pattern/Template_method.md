...
- 子类必须遵循父类设计，不够灵活。
## 注意
- 在数据库操作中，不同类型的数据库（如MySQL、Oracle、SQLServer等）都需要经过连接、打开、使用、关闭这些固定的步骤，但每种数据库在连接方式上存在差异。模板方法模式可以在抽象类中定义这些操作的框架，而将特定的连接实现留给具体的子类完成。
- 模板方法模式最适合实现这种具有固定操作步骤但细节实现可能不同的场景。
